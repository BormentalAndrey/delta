...
public class SwapAnimation implements IAnimation {
    ...
    public void update(float delta) {
        ...
        float deltaX = this.distanceX * currentDelta * totalDurationInv;
        float deltaY = this.distanceY * currentDelta * totalDurationInv;
        ...
        if (this.currentDuration >= totalDuration) {
            if (this.swapBack) {
                ...
                // Рекурсивный вызов для остатка дельты
                if (delta - currentDelta > 0) this.update(delta - currentDelta);
            } else {
